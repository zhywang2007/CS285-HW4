    ...
    def update(
        ...
        for _ in range(cfg.ppo_epochs):
            for mb in iter_minibatches(
                ...
                adv=adv.unsqueeze(1)
                unclipped = ratio * adv
                clipped = torch.clamp(ratio, 1.0 - cfg.ppo_clip_eps, 1.0 + cfg.ppo_clip_eps) * adv
                per_token_obj = torch.min(unclipped, clipped) * mask
                ...
                clipfrac = ((ratio > (1.0 + cfg.ppo_clip_eps)) | (ratio < (1.0 - cfg.ppo_clip_eps))) * mask
                clipfrac = masked_mean(clipfrac, mask)
